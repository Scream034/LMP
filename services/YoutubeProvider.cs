...
public class YoutubeProvider
{
    ...
    private static readonly Regex YoutubeMusicRegex = new(
        @"music\.youtube\.com",
        RegexOptions.Compiled | RegexOptions.IgnoreCase);
    ...
    public async Task<List<TrackInfo>> GetRadioAsync(TrackInfo sourceTrack, int count = 25)
    {
        ...
        try
        {
            ...
            // Помечаем треки как часть радио
    ...
    public async Task<List<TrackInfo>> GetTrendingAsync(int count = 20)
    {
        ...
        try
        {
            // Используем YouTube Music чарты
    ...
    public async Task<List<TrackInfo>> GetPersonalRecommendationsAsync(int count = 20)
    {
        ...
        try
        {
            // Получаем историю и лайкнутые видео для рекомендаций
            _http.DefaultRequestHeaders.Authorization = 
                new AuthenticationHeaderValue("Bearer", token);
            ...
            if (data.TryGetProperty("items", out var items))
            {
                foreach (var item in items.EnumerateArray())
                {
                    ...
                    // Получаем полные данные через yt-dlp
            ...
            // Если мало результатов, добавляем радио на основе первого трека
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            var options = new OptionSet
            {
                ...
                AudioQuality = 0,
                Output = Path.Combine(_ytdl.OutputFolder, "%(title)s.%(ext)s")
            };
